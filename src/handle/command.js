...
module.exports = async (client, message) => {
    ...
    if (!timestamps.has(member.id)) {
        ...
        if (now < expirationTime) {
            ...
            return message.channel.send(`**${member.user.username}**, mohon tunggu **${timeLeft.toFixed(1)}** detik.`).then(msg=>msg.delete(3000));
        }
  ...
  console.info(`${message.author.tag}[${message.author.id}] menggunakan ${message.content.split(" ")[0].replace(prefix, '')} command on ${message.guild.name}[${message.guild.id}]`);
  }
}
