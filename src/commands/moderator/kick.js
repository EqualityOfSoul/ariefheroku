...
return message.channel.send(embed);
  ...
  if (!message.guild.member(client.user).hasPermission("MANAGE_NICKNAMES")) return message.channel.send(`**${message.author.tag}** Maaf, KuuHaku Tidak Mempunyai Permissions \`KICK_MEMBERS\` Tolong Beri Permissions \`KICK_MEMBERS\` Terlebih Dahulu.`).then(msg=>msg.delete(5000))
  ...
  if (toKick.hasPermission("KICK_MEMBERS")) return message.channel.send("Hmm, Dia Tidak Dapat Dikick :(");
