...
exports.run = async(client, message, args) => {
    ...
    if (!args[0]) {
        const ghembed = new Discord.RichEmbed()
            .setColor("RANDOM")
            .setTitle(`${message.author.username} memeluk diri sendiri...! Aneh..?`)
    ...
    if (!message.mentions.members.first().user.username === message.isMentioned(message.author)) {
        const hembed = new Discord.RichEmbed()
            .setColor("RANDOM")
            .setTitle(`${message.author.username} memberi ${message.mentions.members.first().user.username} pelukan ! Wah.. manisnya!`)
...
exports.help = {
    name: "hug",
    description: "Memberikan seseorang sebuah pelukan",
    usage: "hug <@mention>"
}